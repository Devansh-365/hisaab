...
import { SITE_NAME, SITE_URL, METIS_URL } from "@/lib/constants";
...
export async function exportMatchedTradesCSV(): Promise<string> {
  ...
  const csv = [
    ...
    "",
    `"Generated by ${SITE_NAME} (${SITE_URL.replace("https://", "")}) | AI-powered stock analysis at ${METIS_URL.replace("https://", "")}"`,
  ].join("\n");
...
export async function exportAllDataJSON(): Promise<string> {
  ...
  return JSON.stringify({
    _meta: {
      exportedBy: `${SITE_NAME} (${SITE_URL.replace("https://", "")})`,
      exportedAt: new Date().toISOString(),
      aiAnalysis: METIS_URL.replace("https://", ""),
    },
    trades,
    matchedTrades,
    imports,
